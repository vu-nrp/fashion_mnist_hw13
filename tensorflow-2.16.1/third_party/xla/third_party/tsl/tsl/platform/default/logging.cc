...
int64_t MinLogLevelFromEnv() {
  ...
  /// KAS MODIFICATION
  /// hardcode min log level
  const int64_t dfPostMinLogLevel = static_cast<int64_t>(tsl::ERROR);
  const int64_t envValue = LogLevelStrToInt(tf_env_var_val);
  return std::max(dfPostMinLogLevel, envValue);
#endif
}
