...
void ComposeSignature(const OpDef& first_signature,
  ...
  *fused_signature->mutable_output_arg() = second_signature.output_arg();
  ...
  for (auto it = first_signature.control_output().begin(); it != first_signature.control_output().end(); ++it)
      *(fused_signature->mutable_control_output()->Add()) = *it;

  for (auto it = second_signature.control_output().begin(); it != second_signature.control_output().end(); ++it)
      *(fused_signature->mutable_control_output()->Add()) = *it;
}
