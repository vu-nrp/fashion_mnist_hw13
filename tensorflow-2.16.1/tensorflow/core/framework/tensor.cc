...
void LogUnexpectedSize(int64_t actual, int64_t expected) {
  LOG(FATAL) << "Input size was " << actual << " and expected " << expected;
}
  ...
  if (in_n <= 0) {
    ...
    for (int64_t i = 0; i < real_n; ++i) {
      Status s = ResourceHandle::BuildResourceHandle(in.resource_handle_val(i),
                                                     &data[i]);
      if (!s.ok()) {
        LOG(FATAL) << "Could not decode resource handle from proto \""
  ...
  if (in_n <= 0) {
    ...
    for (int64_t i = 0; i < real_n; ++i) {
      ...
      if (!DecodeUnaryVariant(&data[i])) {
        LOG(FATAL) << "Could not decode variant with type_name: \""
                   << data[i].TypeName()
...
int Tensor::RefCount() const {
  if (buf_->root_buffer() != buf_) {
    LOG(FATAL) << "Tensor RefCount not reliable if buf_ points to a SubBuffer.";
    return -1;
  }
