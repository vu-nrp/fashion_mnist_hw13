...
void AppendProtoDebugString(
  ...
  o->AppendStringIfNotEmpty("float8_val", ProtobufStringToString(msg.float8_val()));
}
...
bool ProtoParseFromScanner(
    ...
    ::tensorflow::TensorProto* msg) {
  std::vector<bool> has_seen(18, false);
  while(true) {
    ...
    if (identifier == "dtype") {
      ...
      if (value == "DT_INVALID") {
        ...
        msg->set_dtype(::tensorflow::DT_FLOAT8_E5M2);
      } else if (value == "DT_FLOAT8_E4M3FN") {
        msg->set_dtype(::tensorflow::DT_FLOAT8_E4M3FN);
      } else if (value == "DT_INT4") {
        msg->set_dtype(::tensorflow::DT_INT4);
      } else if (value == "DT_UINT4") {
        msg->set_dtype(::tensorflow::DT_UINT4);
        ...
        msg->set_dtype(::tensorflow::DT_FLOAT8_E5M2_REF);
      } else if (value == "DT_FLOAT8_E4M3FN_REF") {
        msg->set_dtype(::tensorflow::DT_FLOAT8_E4M3FN_REF);
      } else if (value == "DT_INT4_REF") {
        msg->set_dtype(::tensorflow::DT_INT4_REF);
      } else if (value == "DT_UINT4_REF") {
        msg->set_dtype(::tensorflow::DT_UINT4_REF);
      } else {
        int32 int_value;
    ...
    else if (identifier == "float8_val") {
      if (has_seen[17]) return false;
      has_seen[17] = true;
      string str_value;
      if (!parsed_colon || !::tensorflow::strings::ProtoParseStringLiteralFromScanner(
          scanner, &str_value)) return false;
      SetProtobufStringSwapAllowed(&str_value, msg->mutable_float8_val());
    }
  }
}
